...
In this section, we will present the `tidyverse`-codestyle by Wickham et al. (2024).
...
In psychology, the grammar of tidyverse is widely used for data wrangling. In this section we will give a short introduction on code style.
...
The `verify()` function take a data frame (which is the first argument of the function and provided by the `|>` operator) and a logical expression.
...
When the expression is `FALSE`, verify will raise an error that terminates any further processing of the pipeline (Fischetti, 2023).
...
`assert`, `assertthat`, and `testthat` are other powerful packages in this context.
In general, code validation is not too popular in scientific practice.
It is rather prevalent in software development contexts.

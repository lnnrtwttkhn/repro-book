...
Functional programming is basically the idea of defining your code for repetitive actions in functions and using these instead of repeating your code line-by-line.  
...
gg_histogram <- function(data = bfi_data, x_axis){
  ggplot(data = bfi_data, aes(x = bfi_data[[x_axis]])) +
  geom_histogram(binwidth = 0.5, fill = "steelblue") +
  labs(x = x_axis) +
  theme_classic()
}
...
gg_histogram(x_axis = "A1")
gg_histogram(x_axis = "A2")
gg_histogram(x_axis = "O5")
```
...
E.g., in the earlier more elaborate example with histograms, the formals are `data=bfi_data` and `x_axis`.

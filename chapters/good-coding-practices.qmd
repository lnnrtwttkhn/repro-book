...
# get histograms for three different items in the bfi_data set, which each contribute to a different personality trait
...
# item O5: Will not probe deeply into a subject

# without using a function, we just copy-pasted the code for each histogram and manually changed the values for the x argument to the respective items A1, A2 and O5
...
This approach does give us three simple histograms of three different items.
But now imagine you realize you want to do this for all 25 different iterations of these items.
...
To give another example, we return to the `bfi_data` data set on the Big Five from the [earlier example](#functional-programming) and instead of only creating the histograms of three items, we create histograms for all 28 variables in the data set, which includes age and gender. 

```{r}
# Create a function 
variable_histogram <- function(var_name) {
  hist(bfi_data[[var_name]],
       main = paste("Histogram of", var_name),
       xlab = "Strength of self-assignment",
       col = "darkgreen")
}

# Make a histogram for each variable, by using the function lapply() to run 
# the function variable_histogram for the argument "names(bfi_data)" which are 
# the names of the variables/items specified in bfi_data.
# names(bfi_data) is the argument for the formal "var_name".
lapply(names(bfi_data), variable_histogram)

...
::: {.column width="49%"}
```{r filename="Relative file path does work"}
#| eval: false
> getwd()
[1] "Users/my-user-name/my-project"
...
:::{.column width="49%"}
```{r filename="Relative file path does not work"}
#| eval: false
> setwd("script")
> getwd()
...
A way to avoid confusions about which file paths to use, is the `here` package [@mueller2020].
The `here()` function works regardless of what your current working directory is.
...
With `here::here()`, it is possible to refer file paths regardless of your working directory.
`here()` uses the top-level directory of a file project to build the paths.
It recognizes special files (e.g. `.Rproj`) and infers a project folder.
Using `here()` referencing file paths will always use the same project structure regardless of the file you are referencing.
...
Another benefit of using `here()` is enhanced readability because the paths do always start at the project directory.
Using relative paths would require the reader to think about the current working directory and how certain files are relatively located to it.
Further, while separating files with characters and commas, here is untouched by different operating systems.

::: {.callout-note title="Path differences between operating systems" collapse="true"}

MacOS and Linux use slash `/` for path separators. In contrast, Windows uses a backslash `\` for path separators.
In R, `\` is an escape operator.
Thus, R will misinterpret Windows backslash `\` when using them as path separators.
For Windows, the use of `\\` or `/` is appropriate in handling path separators.

A benefit of `here()` is that one can specify paths within `""` separated by commata.
Depending on the operating system `here()` applies a correct path separation.
...
::::
### Evaluate your code style

After you have learned the `tidyverse` codestyle, you can check your code if you have any deviations from that codestyle.
This is called "linting" and comparable to a program that checks for spelling errors.
The `lintr` package [@hester2024] can do that for you and analyses your code for...

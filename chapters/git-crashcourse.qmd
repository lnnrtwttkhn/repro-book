...
However, it is quite hard to add files and folders to the `.gitignore` file that have been tracked before.
Consider you have a folder (e.g. `misc`) in your repository that you want to ignore but it was tracked before by git (see @gitignore).
...
You have to use the command line to not track files and folders that have been tracked before.

```{zsh filename="Code for the command line"}
git rm --cache -r folder/path # <1> 
git rm --cache  file/path/document-A.pdf # <2>
```

1. This command leads to Git not tracking the subfolder `path` in the folder `folder` anymore and all the files and folders in the subfolder of `path`.
2. This command leads to Git not tracking the file `document-A.pdf` in the subfolder `path` of the folder `file` anymore.

...
abstract: |
  ...
  You will learn the basics of Markdown syntax, how to write a paper within Quarto, and how to format it for a required output.
  You will learn how to use extensions, specifically the apaquarto extension to write a reproducible paper in APA-style.
---

::: {.callout-tip appearance="minimal"}
<h5>Learning Objectives</h5>

:bulb: Learn the advantages and disadvantages of literate programming.<br>
...
:bulb: Learn more advanced syntax: add a quarto  extension to your quarto document.<br>
...
The code in these documents are stored in **chunks** (see @sec-code-chunks) and are executable.
...
Thus, the paper becomes dynamic (@nte-dynamic-doc).
Moreover, literate programming can heavily reduce outcome-based reproducibility errors (@sec-repro-errors).

::: {#nte-dynamic-doc .callout-note title="Static vs. Dynamic documents" collapse="true"}
...
A prominent example is the [dashboard](https://coronavirus.jhu.edu/us-map) of the [John Hopkins University](https://www.jhu.edu/) that displayed data referring to the Covid-19 Pandemic.
The dashboard was updated every week with new data from all over the world.

In a classical sense, research paper can be seen as static documents, since they will be rarely updated after they were published.
However, since research paper are often reviewed and many changes must be done, one can view paper as dynamic documents before publication.
...
Quarto is well integrated in RStudio and comes already bundled with any version of RStudio from RStudio v2022.07.1 "Spotted Wakerobin" onward.
In case you're using an older version of RStudio, here are instructions on [how to install Quarto](https://docs.posit.co/ide/user/ide/guide/documents/quarto-project.html#installing-quarto).
...
To do so, Quarto has its own file extension (`.qmd`).
`qmd` stands for **Q**uarto **M**ark**d**own.
...
### Source and Visual Editor {#sec-editors}

You can use either the `Source` or `Visual` mode in RStudio to write and format your work.
In the `Source` mode you use [Markdown syntax](@sec-markdown) to format your written work and in the `Visual` mode you use a User-Interface similar to how you would use in e.g., MS Word.
...
However, we strongly recommend you to use the `Source` mode rather than the `Visual` mode.
This is because most Markdown Syntax is easier and faster to write than to use a Graphical User Interface.
Nevertheless, in the Section about Markdown Syntax we give you instructions how to format your text in both editors.
...
::: {#tip-quarto-git-compatibility .callout-tip title="Enhance Quarto compatibility with Git" collapse=true}

When you make changes in a Quarto document and you track it with Git, we recommend you to write each sentence in a new line when using the `Source` Editor.
This enhances the readability of the changes in Git you made in the Quarto document but does not change the output of the document (see @fig-quarto-git-compatibility).

![The upper left corner displays text. Lines 6 to 8 contain a paragraph where each sentence is written in a new line. Line 10 contains a paragraph where all sentences are written in one line. The lower left corner displays the rendered `html` version of the quarto document. The right half shows the popup window of Git to commit changes. As one can see, it is easier to understand the changes that were made in line 8 compared to line 10.](../static/quarto-git-compatibility.png){#fig-quarto-git-compatibility}

:::

::: {#tip-line-wrapping .callout-tip title="Avoid changes when switching between `Source` and `Visual` Editor" collapse="true"}

Assuming you follow @tip-quarto-git-compatibility, and you write each sentence in a new line.
When you use the `Visual` Editor, you are cannot directly control whether the sentences are in separate lines or not (compare @fig-quarto-git-compatibility).
However, you can specify that in the [YAML header](@sec-yaml-header) (see the code snippet below).
...
This formatting is different to the so-called **WYSIWYG** (*What you see is what you get*) editors, like Word.
There are some advantage over these more commonly known editors.
...
::: {.callout-tip title="How to start a new page in docx and pdf files" collapse=false}

If you want to start a new page in your document at a specific spot, e.g., to have your references on their own page, you can add a pagebreak.
The pagebreak option does only make sense when you render your document to `docx` or `pdf`, since these files consist of multiple pages.
However, this does not apply to `html` files, because you can just scroll down the content.
To add a pagebreak, simply add it between your intended pages like below and a new page will start in the rendered document.
...
You can also edit your headings in the menu line of the Visual Editor via click and select.
Simply choose between normal text or your desired header level.

![This is where you can find the different header options in the Visual Editor.](../static/markdown-visual-header.jpg){#fig-visual-header}

:::

### Lists

There are many different types of lists available in Markdown and they are very easy to implement in Source Mode.
Take a look below at the different options and how they change when rendered.
...
You have access to the same types of lists in the Visual as in the Source Editor.
when wanting to create a list, head to the menu line under **Format > Bullets & Numbering**.
Some of the options only become available once you've created a list and want to edit it, like `Tight List`or `Sink Item`.

![This is where you can find the different list options in the Visual Editor.](../static/quarto-visual-lists.jpg){#fig-visual-lists}
:::

### Tables

You can also create tables using Markdown.
However, this is one case where using either the Visual Editor or even better a Markdown table creator is recommended.
Doing this in the Source Editor is very tedious and in terms of speed will leave you hanging behind the Visual Editor.
However, in case you want to give it a try or come across it in a Markdown document, take a look below at how to create a table in the Source Editor.

::: panel-tabset

#### Source Editor

Each new line is also a new row in the table and the columns are created using `|` as separators.
You can define the width of a column using `-` and define where the text should be centered using `:`.
...
To create a table in the Visual Editor, head to the menu at the top again and go to **Table > Insert Table...**.
This will open a prompt that asks you about the general dimensions of your table but you can still change these later on.
You also have the option to include a caption at this point.
Once you've created a table more options like centering or adding more rows will become available.

![This is where you can find the different table options in the Visual Editor.](../static/quarto-visual-table.jpg){#fig-visual-table}

:::

### Equations

A very nice feature of Quarto is that it allows you to easily include equations.
Regardless of which editor you use, you have to write the equations within LaTeX style.
For a guide how to create sophisticated equations (including fractions, matrices, square roots, integrals, etc.), see the respective chapter in the [LaTeX Beginner's Guide](https://latexguide.org/book-contents/chapter-09-writing-math-formulas/).

::: panel-tabset

#### Source Editor

To include an equation you need to frame it with dollarsigns `$`.
One dollarsign for inline equations `$` and two for a standalone equation `$$`.
...
![This is where you can find the two different options for equations in the Visual Editor.](../static/quarto-visual-equations.jpg){#fig-visual-equations}
:::

### Blockquotes

You can insert blockquotes in your Quarto documents.
Whilst this is likely not syntax you will need a lot in scientific reports, it's very easy to do and might prove useful when taking notes or formatting a website you're creating.
...
![This is where you can find the blockquote option in the Visual Editor.](../static/quarto-visual-blockquote.jpg){#fig-visual-blockquote}

:::

### Links & Images

You can easily insert online links or images into your document.
The process to do this is relatively similar, which is why the two actions are in the same section.
...
And this is the resulting **output**: [Quarto](https://quarto.org/)

Inserting an **image** works similarly.
Let's say you want to insert an image that you've found online, in this case the Quarto logo.
To do this, start off with an exclamation mark, followed by squared brackets, which can contain a caption for your image, but you can otherwise leave empty, followed by the link to the image in round brackets `![Optional caption](Image link)`.
...
You can of course also link to images you've taken yourself or that aren't stored online by inserting a local image path instead of an online link.
If the image is stored in the same subfolder as your document, you can refer to it directly, however it is recommended to create a designated image folder instead, to keep your project structure clear to others and your future self.
...
When using images or figures in e.g., a report about an analysis you've done, to not use a fixed image of it. Rather use a [code chunk](#code-chunks), which you will learn about in a little bit, to create it and keep them reproducible.
:::
...
![This window lets you insert links in the Visual Editor in RStudio. Used under a [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0) license.](../static/quarto-visual-link.jpg)

To insert **images** using the Visual Editor, select from the menu line **Insert > Figure/Image...**, which leads you to a pop-up window, in which you can either insert the link to an image or browse your stored images.
In this window you can also add links to websites and a caption for your image.

![This window lets you insert images in the Visual Editor in RStudio. Used under a [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0) license.](../static/quarto-visual-images.jpg)
:::
...
::: {#tip-yaml-settings .callout-tip title="Different YAML settings" collapse="true"}
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| YAML key       | Example                                                      | Description                                                                                                   |
+:===============+:=============================================================+:==============================================================================================================+
| `title`        | ``` {.markdown}                                              | Name the title of your document                                                                               |
|                | title: "The Repro Book - A Guide to Reproducible Research"   |                                                                                                               |
|                | ```                                                          |                                                                                                               |
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| `shorttitle`   | ``` {.markdown}                                              | The short title of your document                                                                              |
| `shorttitle`   | shorttitle: "The Repro Book"                                 |                                                                                                               |
| `shorttitle`   | ```                                                          |                                                                                                               |
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| `author:`      | ``` {.markdown}                                              | The author(s) of your document and their affiliation(s) and address.                                          |
|                | author:                                                      |                                                                                                               |
|                |   - name: Justus J. Reihs                                    | You can include as many or as little of these pieces of information in your author description as you desire. |
|                |     email: justus.reihs@studium.uni-hamburg.de               |                                                                                                               |
|                |     corresponding: true                                      |                                                                                                               |
|                |     affiliations:                                            |                                                                                                               |
...
|                | ```                                                          |                                                                                                               |
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| `editor`       | ``` {.markdown}                                              | Set that each sentence is placed in a new line (@tip-line-wrapping)                                           |
|                | editor:                                                      |                                                                                                               |
|                |   markdown:                                                  |                                                                                                               |
|                |     wrap: sentence                                           |                                                                                                               |
|                | ```                                                          |                                                                                                               |
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| `csl`          | ``` {.markdown}                                              | Set the citation style of your document                                                                       |
|                | csl: apa.csl                                                 |                                                                                                               |
|                | ```                                                          |                                                                                                               |
+----------------+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| `bibliography` | ``` {.markdown}                                              | Refer to the citations you use in your document                                                               |
...
Remember the earlier example of a plot within a report, which was done using a code chunk.
...
:::{.callout-tip title="Make a code chunk interactive" collapse=false}
You can make R code chunks interactive for your readers by installing an extension.
This can be a very useful and fun feature in e.g., teaching scenarios.
You can learn how to do this [later in this chapter](#sec-executable-code-chunk).
:::
### Inline code

Inline code allows you to simply insert code as part of your other markdown syntax.
This is especially useful for referencing analysis outputs.
In keeping with reproducibility this ensures that if you change parts of your analysis, your results will update as well without you having to manually go into the text change them manually, risking that you might miss some.
Furthermore, when writing your report, you don't have to go back and forth, checking what your specific results were but instead can just refer to them and move on with writing.  
To add inline code, it's the same process as for inserting a code chunk but instead of three backticks, you just use one.
Remember to include the programming language in `{}`.
...
You can cross-reference objects like figures, tables or equations within your text.
This is especially useful in reports when you need to refer to different figures.
The references work as links to the objects and let you jump to them in the final document.

For example: @sec-citations (`@sec-citations`) brings you to the section on citations in this book.

To be able to cross-reference tables, sections of your text, equations or figures, they need to have a label.
You need to add the label after the inserted object.
The label is in curly brackets and start with a hash `#` like so `{#object-name}`.
For example, for an image this looks like this: `![A fish](fish-image.jpg){#fig-fish}`.
To reference it in the text you loose the curly brackets and replace the hash with an at-sign `@`, which looks like this `As displayed in @fig-fish, this a fish.`.

Avoid using underscores `_`when creating your labels as this might cause problems when rendering to PDF. 
There is a set of predefined prefixes you can and should use to refer to your objects.
Then just follow this with a name of your choosing.
...
We will give you an overview on how to handle them.
...
To use the citation style language you downloaded with the `.csl` file, you have to make sure that the intended `.csl` file appears in the *root directory* of the project[^2].
...
[^2]: You can also put it in a subfolder of your project, but then you have to specify the path in the YAML header, too.
...
In this book, we will only deal with two extensions - the `apaquarto` extension by William Joel Schneider and the `webR` extension for Quarto, which will get to later.
With the `apaquarto` extension, we can shape the output of our rendered quarto file into the format of the [APA guidelines](https://apastyle.apa.org/)[^literate-programming-3] without much effort.

[^3]: Many journals and universities require submissions to be in the APA format.
...
After you have installed all things, you can successfully install `apaquarto`.
...
### The Quarto webR-extension and executable code chunks{#sec-executable-code-chunk}

Another great extension is the [`webR`- extension for Quarto](https://quarto-webr.thecoatlessprofessor.com/), which allows you to include **executable code chunks** in your documents.
This makes code chunks **interactive**.
An executable code chunk is **not** already executed in the rendered document but instead needs to be prompted to run by the reader.
Whilst this might sound a bit strange at first - Why would you make a reader execute a code chunk and not just have it be run already? - this can be a great tool in teaching or presentation scenarios.
It allows your readers to see and better understand the order in which code needs to be run, what part of the code leads to what outcome, change parameters within and the code itself and see how this changes intermediate and final outputs.
They can easily explore data and better understand mechanisms without having to leave the document or site that is explaining these mechanisms to them.

See the executable code chunk below.
You might recognize the code from our earlier example.
Go ahead and run it by pressing the `> Run Code` button.  
Take some time and **play around** with the code.
Try and edit the code, e.g., change values, swap the color or title of the graph and run the code chunk again.

```{webr-r}
x <- 1:10
y <- x^2

plot(x, y, type = "b", col = "blue", main = "Basic Plot", xlab = "X values", ylab = "Y values")
```

##### Adding executable code chunks

Beware that this feature is only available for **code in the programming language R** and for the Quarto output formats HTML, RevealJS (presentations), Websites, Blogs and Books (e.g., this site is a Quarto book).  
To get started with adding executable code chunks to your documents, you'll need to complete a couple things first.

**1. Install the Quarto webR-extension**.  
Make sure you're in the directory that you'll later add the Quarto documents with the executable code chunks to.
You'll have to add the extension seperately to each directory that you want to use it in!  
Then, run the code chunk below in your terminal in RStudio.
```{zsh filename="Terminal"}
quarto add coatless/quarto-webr
```

Below you can see what your terminal will look like prompting you through the installation and you agreeing to the changes.
Answer Yes/Y when asked to trust the author and to install the webR-extension, similar to when [installing the apaquarto extension](#install-apaquarto).
Lastly, it will ask you if you would like to look at the documentation, which you can answer Yes/Y to, but really don't have to.
```{zsh filename="Output"}
...
**2.** Open a **new .qmd document**, selecting e.g., HTML as the output.  
**3.** **Change the document's [YAML-header](#sec-yaml-header)** to include the following:
```{r}
filters:
  - webr
```
**4.** Now you can actually start to **add executable R code chunks**, by simply specifying `{webr-r}` instead of `{r}`at the top of a code chunk.
As an example, a code chunk could look like this:
...
**5. Render** your document as usual and see your newly added feature in action.
```{webr-r}
#| echo: false
x <- 1:10
y <- x^2
...
:::{.callout-caution title="Install the *webR*-extension for each directory!" collapse=false}
Unfortunately you need to go through the installation process for each directory that you want to use it in.
You need to install the extension every time you want to use it in a project and are in a new directory.
As the installation of the extension is pretty quick, this shouldn't prove to be too much trouble but just needs to be remembered and has to be done once per directory.
:::

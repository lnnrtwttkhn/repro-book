...
::: {#tip-quarto-git-compability .callout-tip title="Enhance Quarto compability with Git" collapse=true}
...
This enhances the readability of the changes in Git you made in the Quarto document but does not change the output of the document (see @fig-quarto-git-compability).

![The upper left corner displays text. Lines 6 to 8 contain a paragraph where each sentence is written in a new line. Line 10 contains a paragraph where all sentences are written in one line. The lower left corner displays the rendered `html` version of the quarto document. The right half shows the popup window of Git to commit changes. As one can see, it is easier to understand the changes that were made in line 8 compared to line 10.](../static/quarto-git-compability.png){#fig-quarto-git-compability}

:::

::: {#tip-line-wrapping .callout-tip title="Avoid changes when switching between `Source` and `Visual` Editor" collapse="true"}

Assuming you follow @tip-quarto-git-compability, and you write each sentence in a new line.
When you use the `Visual` Editor, you are cannot directly control whether the sentences are in separate lines or not (compare @fig-quarto-git-compability).
...
You have access to the same tyoes of lists in the Visual as in the Source Editor.
...
In keeping with reproducibility this ensures that if you chnage parts of your analysis, your results will update as well without you having to manually go into the text change them manually, risking that you might miss some.

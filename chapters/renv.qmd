...
The next chapters will dealth with reproducible computational environments.
...
The longer the time interval between creation and reproduction of your code is, the more the more likely reproducibility is not possible because of different computational environments.
...
In the next two chapters, we will introduce two Package Mangament Systems, namely {renv} and {docker}.
...
::: {.callout-tip title="Section of the helping page for renv::status()" collapse="true"}
...
- If it's not used and not recorded, there's nothing to do.
This the most common state because you only use a small fraction of all available packages in any one project.
...
In the helping page our case is well-described.
...
::: {.callout-tip title="Video explaining basic `renv` functions"}
...
Another way is using *docker* (see chapter about docker).

...
However, human readability also increased.
In the box below, you can find guidelines facilitating making your data FAIR.

::: {.callout-tip title="Guidelines of **FAIR** data"}

These guidelines are taken from A FAIRy tale, a book providing an understandable explanation of each principle [@hansen2018].

::: panel-tabset
## Findable

1. Use a **persistent identifier** as a DOI for your research project.
Thus, your data becomes findable for everyone with the DOI.
Data platforms as [Zenodo](https://zenodo.org/) provide easy tools to do that.

1. Provide **rich metadata**. 
Metadata are often the first point of contact in search engines, when humans and machines look for data.
Metadata should contain information about context, instruments used in the study, sample size, etc.
These details make your data findable, thus, creating a bigger impact on research.

1. Metadata should **include the identifier** of the data.
Metadata and data are usually treated separately.
Therefore, a link between Metadata and data must be included.
Without, the metadata nicely describe data that cannot be found.

1. Meta(data) are registered or indexed in a **searchable resource**.
Structured searches for datasets usually involve search engines.
It does not help your data to be found on project websites.
Your data can have more impact when it is found in a repository using a structured way of building metadata.
For psychology, good repositories are [OSF](https://osf.io/) and [Zenodo](https://zenodo.org/).
...
1. Retrieval is possible by **standardised protocol**.
Not all data is *openly* available.
When you cannot provide your data to the public of the internet, you should have a standardised protocol that determines how someone can have access to your data.
That could be contact information or any additional mechanisms.
This might be particularly important when you want to publish data in clinical psychology.
...
However, that data does not contain the data itself.
...
That leaded to misunderstanding and time wasted on rearranging data and possible causes for errors.
